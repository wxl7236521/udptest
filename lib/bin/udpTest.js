...
let StreamReader = function (bufferLimit) {
    let _bufferLimit = bufferLimit || (1024 * 32);
    let self = this;
    let _buffers = []; //缓存数组，每个节点为一个push传入的一个Buffer
    let _curBuffer; //当前正在
    let _curBufferOffset = 0;
    let _bufferDataTotal = 0;
    let _tasks = []; //读取任务{type: 'once' | 'loop', total: xx, packLen:yy, index:zz, cbFunc: function() }
    let _curTask;
...
let Writer = function () {
    ...
    this.pack = function () {
        ...
        let ret = Buffer.alloc(len); //Alloc result
        let offset = 0;
        //Package result
        for (let i = 0; i < _targetList.length; i++) {
            let item = _targetList[i];
...
class udpTest {
    constructor(numPacket, sendCount, perCount, interval, ipServer, portServer, isClinet, portClinet, ipv4oripv6) {
        this.isClinet = true;
        this.ipv4oripv6 = 'udp4';
        this.currentReceive = new Map();
        this.lostReceive = new Map();
    ...
    sendData(id) {
        if (this.isClinet) {
            ...
            for (let i = 0; i < bytecount; i++) {
                loopStr += 'This is a test loop string apple';
            }
            let loopStr2 = ''; //1K
            for (let i = 0; i < this.numPacket; i++) {
                loopStr2 += loopStr;
            }
    ...
    init() {
        ...
        const that = this;
        that.client = dgram_1.default.createSocket({ type: that.ipv4oripv6, reuseAddr: true });
        if (that.isClinet) {
        ...
        that.client.on('message', function (msg, info) {
            ...
            if (that.sockStreamReader.taskEmpty())
                that.registerProtocolBody();
    ...
    registerProtocolBody() {
        const that = this;
        that.sockStreamReader.read(17, function (headData) {
            ///解析头
            ...
            switch (head.type) {
                ...
                case PACK_TYPE_STR:
                    ...
                    // eslint-disable-next-line no-case-declarations
                    let total = 0;
                    that.sockStreamReader.loopRead(head.length, head.length, function (data) {
                        const dataStr = data.toString();
